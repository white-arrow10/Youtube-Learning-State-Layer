...
export function ReentryOverlay({ onResume, onRecap }: Props) {
  ...
  return (
    ...
    >
      <div className="relative max-w-4xl ml-6" style={{ marginTop: 'calc(56.25vw * 0.55 + 24px)', maxWidth: 'calc(100% - 344px)' }}>
        <div
          className={`overlay-card px-6 py-5 max-w-lg ${dismissing ? 'animate-fade-out-down' : 'animate-fade-in-up'}`}
        ...
        >
          ...
          <p className="text-[15px] text-foreground leading-relaxed mb-5">
            You left off at <span className="font-medium text-primary/90">47:23</span> — <span className="font-medium">Newton's Second Law (F=ma)</span>
          </p>

          <div className="flex items-center gap-3">
            <button onClick={onResume} className="pill-button pill-primary">
              Resume
            </button>
            <button onClick={onRecap} className="pill-button pill-secondary">
              Quick Recap
            </button>
          </div>
